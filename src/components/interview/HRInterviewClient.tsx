...
import HRQuestionArea from "./HRQuestionArea";
import InterviewTimer from "./InterviewTimer";
import ProgressBar from "./ProgressBar";
import NotesSection from "./NotesSection";
import FeedbackPanel from "./FeedbackPanel";
import HREmotionIndicator from "./HREmotionIndicator";
import { Button } from "@/components/ui/button";
import type { LiveHRQuestion, HRAnswerEvaluation, InterviewState, ChatMessage } from "@/types";

interface HRInterviewClientProps {
  sessionId: string;
}

export default function HRInterviewClient({ sessionId }: HRInterviewClientProps) {
  const router = useRouter();

  // State Machine
  const [state, setState] = useState<InterviewState>("idle");
  const [currentQuestion, setCurrentQuestion] = useState<LiveHRQuestion | null>(null);
  const [draftAnswer, setDraftAnswer] = useState("");
  const [evaluation, setEvaluation] = useState<HRAnswerEvaluation | null>(null);
  const [evaluationsHistory, setEvaluationsHistory] = useState<
    Array<{ confidenceScore: number; communicationScore: number }>
  >([]);
  ...
  const fetchNextQuestion = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/v1/sessions/${sessionId}/hr-question/next`, {
        method: "POST",
      });
      const data = await readJsonSafely(res);
      ...
      const questionData = data.data as LiveHRQuestion;
    ...
    async (finalText?: string) => {
      ...
      try {
        const res = await fetch(
          `/api/v1/sessions/${sessionId}/hr-question/${currentQuestion.questionId}/answer`,
          {
            method: "POST",
        ...
        const evalResult = data.data as HRAnswerEvaluation;
  ...
  // Compute rolling average scores for HREmotionIndicator
  const avgConfidence =
    evaluationsHistory.length > 0
      ? Math.round(
          evaluationsHistory.reduce((sum, item) => sum + item.confidenceScore, 0) /
  ...
  const avgCommunication =
    evaluationsHistory.length > 0
      ? Math.round(
          evaluationsHistory.reduce((sum, item) => sum + item.communicationScore, 0) /
            evaluationsHistory.length
        )
      : undefined;
      ...
      <div className="flex h-[400px] flex-col items-center justify-center space-y-4">
        {error ? (
          ...
          <>
            ...
            <p className="text-sm text-muted-foreground">Initializing live HR interview session...</p>
          </>
        )}
      </div>
    );
  ...
  return (
    <div className="flex h-[calc(100vh-130px)] flex-col gap-6">
      {/* Header Bar */}
      <div className="flex items-center justify-between glass-card rounded-xl p-4 shrink-0 border-orange-500/20">
        <div className="flex w-1/3 items-center">
          <ProgressBar
            current={currentQuestion?.questionNumber ?? 1}
            total={currentQuestion?.totalQuestions ?? 8}
          />
        </div>
      ...
      <div className="grid flex-1 grid-cols-1 gap-6 lg:grid-cols-12 min-h-0">
        {/* Left Column: Chat, Voice & Emotion */}
          ...
          <div className="shrink-0 flex gap-4">
            ...
            <div className="flex-1">
              <HREmotionIndicator
                ...
                isActive={state === "answering" || state === "submitting_answer"}
              />
            </div>
          </div>
        </div>
          ...
          <AnimatePresence mode="wait">
            {state === "showing_feedback" && evaluation ? (
              <FeedbackPanel
                ...
                onNext={handleNext}
                isLastQuestion={
                  (currentQuestion?.questionNumber ?? 8) >= (currentQuestion?.totalQuestions ?? 8)
                }
              ...
              <div className="flex flex-col gap-6 flex-1 min-h-0">
                <div className="shrink-0">
                  <HRQuestionArea
                    ...
                    starPrompt={currentQuestion?.starPrompt}
                  />
                </div>
                <div className="flex-1 min-h-0 flex flex-col gap-4">
                  <NotesSection
                    ...
                    title="Your Answer"
                    placeholder="Type your response here. Feel free to structure your thoughts using the STAR method: Situation, Task, Action, Result. You can also use the voice panel on the left to transcribe your speech..."
                  />
