...
import { evaluateHRAnswer } from "@/lib/hr-interview-agent";
...
export async function POST(req: NextRequest, context: RouteContext) {
  ...
  try {
    ...
    // Evaluate with Gemini (HR-specific)
    const evaluation = await evaluateHRAnswer(
      questionEvent.questionText,
      transcript || "",
      questionEvent.agentRole || "Behavioral", // agentRole stores the HR category
      difficultyLabel
    );
    ...
    console.error(`POST /api/v1/sessions/${id}/hr-question/${qid}/answer error:`, error);
